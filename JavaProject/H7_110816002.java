...
/*姓名: 張甄真
  ...
  操作說明: 刻出MS Window小算盤介面，並模擬window小算盤的顏色及按鈕擺放位置等，提供使用者基本的加、
          ...
          並直接顯示結果於Label上，且須直接按下C清除本次運算才能繼續下一次的計算。(因為使用到小數點
          運算，我還沒想到陣列儲存值怎麼轉換顯示小數...)故先以單次運算交作業。若要結束小算盤程式可直接按
          右上角的叉叉關閉視窗。
  自評：
      1.程式有意義且可以執行 (+20%)
      2.僅支援整數運算 (+30%)
      3.支援小數運算 (+25%)
      4.包含所有功能 (+25%)
      根據程式能達到評分標準1、2、3的要求，但不確定包含所有功能是指特殊運算按鈕也包括在內，
      或是只是要加減乘除四項功能等，但因程式部分可以達到其他除了基本運算的功能，但特殊運算不是很完整，
      依配分比例故自評分數為95分
